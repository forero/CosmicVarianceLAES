...
From the matching of number density statistics we find that all
the succesful models have halos in the range
$10^{10}-10^{12}$. Including strict criteria and the information from
the angular correlation function does not narrow this mass range, but
help us nevertheless to narrow down the number of good models.





matter masses of halos hosting LAEs is $\log_{10}M_{\rm
  med}=10.9^{+0.5}_{-0.9}$\Msun, with a corresponding occupation
fraction of $1-10\%$.  Our results are in a good agrement with those estimates for the
host mass. This is not unexepected given that we have also
required consistency with ACF measurements. 

Together with the escape fraction $f_{\rm occ}$, quantifying the halo mass range
$\Delta M=\log_{10}M_{\rm max} - \log M_{\rm  min}$ for the succesful
models help us to classify them into three families:
...
1$. Such models imply that there is a cut for lower and higher halo
masses that render inefficient the presence and/or detection of LAEs.
A cut at the low mass end can be readily interepreted in terms of the
minimal halo star formation rate needed to produce the necessary \lya
luminosity to be above a given detection threshold.  However, under the reasonable assumption of star formation rate
increainsng with halo mass, the cut at higher halo masses requires a
different explanation. 

A possible interpretation can be made in terms of a drecreasing escape
...
where the most massive systems are not bright \ly sources \citep{ForeroRomero2012}.  



We find that $70\%$ of the best models have a low
occupation fraction $f_{\rm occ}\leq 0.3$. 

With all the observational constraints we we end up with 50
models out of the initial 90000 combination of parameters. In what
follows we classify the resulting models into three families and
discuss them under the light of previous observational analysis from
blind galaxy surveys and other LAEs clustering analysis. 

\subsection{Constraints from blind surveys}


This preference goes in
...
\cite{Dijkstra2013} use a similar principle to derive their results. They
...
Since the observational estimates based on the ACF infered from photometric
measurements in the Extended Chandra Deep Field South
\citep{Gawiser07} it is a well accepted fact that the median dark
matter masses of halos hosting LAEs is $\log_{10}M_{\rm
  med}=10.9^{+0.5}_{-0.9}$\Msun, with a corresponding occupation
fraction of $1-10\%$.  Our results are in a good agrement with those estimates for the
host mass. This is not unexepected given that we have also
...
host halo mass range. Following the numbers shown for the three
families in the previous section, we see that two thirds of the best models
have a narrow mass range, less than $<1.0$ dex, out of the $2.5$ dex
spanned by dark matter halos at that redshift. Only one fourth of the
models indicate a wide mass range with $>1.0$dex.   

As a byproduct of our results, we are now able to demonstrate that a
simple clustering analysis is not enough to rule out models with a
high occupation fraction $f_{\rm occ}>0.3$, they represent almost
one fourth of our best models, coinciding with a wide range in halo
masses $\Delta M>1.0$ dex. 
...
%their star formation rate is in the range $0.6\pm 0.2$ Msun
...
%Most of the prefered models have a halo mass range with $M_{\rm
%  max}<10^{11.4}$\hMsun. This suggests that a detailed and careful

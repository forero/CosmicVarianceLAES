...
statistics for the number density cosmic variance between different
observed and mock fields. We build a model for LAEs on top of a large N-body
cosmological simulation (Bolshoi) where the basic principle is that a dark
matter halo can only host one LAE with a probability $f_{\rm occ}$ if its
...
\subsection{Additional observational constraints on the occupation fraction}
...
Using the results from the previous sections it is possible to infer
...
result against the following estimation $b^{2}\sim {\sigma_{N}^{2} -
...
As a prediction, we calculate the mean angular correlation function (ACF) for some of the best
models for a mock survey using the match and random methods with 
two different surface areas: $1)$ the full survey (12 subfields) and
$2)$  e SSA22 field (7 subfields) reported by Yamada et al 2012 
(see section XX) . The results for these calculations  are shown 
in figure \ref{figure:correlation_match} (match) and figure
\ref{figure:correlation_random} (random).  The error bars in these figures
represent the standard deviation of the ACF over the subfields. 
We compare these results with the observed ACF in a field of the the SSA22 region
reported by Hayashino et al in 2004.

We can generally observe that the standard deviation of the computed ACF 
in the subfields increase when  increasing $M_{min}$ following the same 
trend as in figure \ref{figure:laes_dist}.Therefore, as the cosmic variance
increase when increasing the halos mass the increment in the ACF standard 
deviation when increasing $M_{min}$ is a direct consequence of cosmic variance.

Given that the ACF reported by Hayashino et al en 2004 is taken over the densest 
field oserved in the SSA22 region by  Yamada et el in 2012 it is expected that
the predicted ACF in the SSA22 region should reproduce this observation. In the 
left panel of figure \ref{figure:correlation_match} we can see the predicted ACFs 
an their corresponding standard deviation over the seven fields that mock the SSA22 
region. It can be seen that the model with $M_min=10.6$ seems to better reproduce the 
Hayashino's ACF and that the corresponding field is in fact an overdense field in the
SSA22 region covered by Yamada et al.
...
match configuration. \label{figure:correlation_match} }
...
\section{Conclusions}
In this \documentname we have constrained the preferred mass for dark matter

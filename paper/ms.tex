...
plausible explanation for the narrow ange in mass $\Delta M<1.0$ dex
...
at high redshift
\citep{Neufeld1991,ForeroRomero2011,Dijkstra2012,Laursen2013,Orsi2012}. 
...
densest field of SSA22 while \cite{Ouchi2008} observed on a $\sim 0.5$
deg$^2$ field.  There are some differences between these
...
different in these references. Nevertheless we use the ACF from these
observations as additional constraints in spite of the fact that the
first selected models are based only on the surface density statistics by
\cite{Yamada2012}.  
...
\label{eq:fitting}
\end{equation}
where $\theta_0$ and $\beta$ are free parametres. The fit is donde
...
cuadratic average of the uncertainties in the fitting procedure. We
...
number density abudance.  A consequence of this variation is that
...
fraction of the modesl can be discartded.

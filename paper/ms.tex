...
We impose a different restriction using the observational results
...
remain constant over that time scale. We limits the escape fraction to
be in the range further pick models that have an occupation fraction of $f_{\rm
  occ}\leq 0.2$ to allow for some flexibility. With this additional
constraint between $35$ to $40$ models are consistent with
observations. 
...
models in the planes $M_{\rm min}-M_{\rm  max}$ and $M_{\rm
  min}-f_{\rm occ}$ for the {\tt match} and {\tt   random}
methods. The list for the parameters in these models is found in the appendix in
Table XX.   
...
\caption{Favored regions in parameter space when the constraints on
  ...
  the {\texttt random} methodology area very similar to the ones
...
\caption{Favored models when the constraints on the maximal number of
  ...
  \label{fig:restriction_mock_and_f_occ}}  
\end{figure*}

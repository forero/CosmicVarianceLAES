...
\begin{abstract}
%
  We study the impact of cosmic variance in constrainng the mass and
  occupation fraction of dark matter halos hosting \ly Emitting
  Galaxies at high redshift. We use and N-body simulation to construct
  mock fields with the same typical size of observed fields at
  $z=3.1$. As a benchmark we match the observed number density
  distribution across fields and the angular correlation function. To
  populate the halos in the simulation we use a model where a dark
  ...
  $M_{\rm  max}<10^{12}$\hMsun\ and $10^{10.2}\hMsun\leq M_{\rm
    min}\leq 10^{11.5}\hMsun$. We also find that the consistent models
  have a narrow mass range, $\Delta M \equiv \log_{10}M_{\rm max} -
  \log_{10} M_{\rm min}$, smaller than $1.0$ dex, with a majority in the range $\Delta M<0.5$ dex. These results
  fall into constraints   already presented in the
...
A secondary benchmark is the angular correlation function
...
The field SSA22 has been known to harbor a significant
...
All the other sub-fields in SSA22 are average. They have a similar
...
Therefore, using the whole SSA22 region as a benchmark in the
...
We recall that we do not perform a explicit modeling for LAE
...
approximation a reasonable one. The lack of close pairs in
  observations \citep{Bond2009}is another hint that the observed LAE
  population agrees with our approximation.
...
The third caveat is the exact value that was used to turn
...
A fourth caveat is the comparison of results for the number density
...
In these models the occupation fraction remains unconstrained, while 
many modelshave a narrow mass range models with $\Delta M\leq 0.5$. 
This approach the discovery of consistent models that have
...
something unexpected from previous studies
\citep[i.e.][]{Gawiser2007,Ouchi2010}  that unanimously favor low
values on the range of $10^{-2}$. The biggest innovation in our
analysis is the construction of mock observations that directly
explore the impact of cosmic variance.

Nevertheless, all the halo mass ranges deduced in previous analysis are a
subset of the models we find in this paper.  All of our preferred
models support the notion that the most massive halos at $z=3.1$ do
not host detectable LAEs. 

In summary, we show that the currently available spatial
...
mass halos with masses spanning less than one order of magnitude. 

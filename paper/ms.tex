...
populations and markers of thethe galaxy formation process through
cosmic history \citep{ForeroRomero2012}.   
...
number density variations among fields are \emph{not} due to different observational
conditions or criteria to construct the catalogs.

The second constraint is the angular correlation function (ACF). The
...
that are resolved with at least $70$ particles (REFF????)
...
The lower limit for the parameter $M_{\rm min}$ IS set by the
observational constraints on the stellar mass $M_{
\rm star}>2\times 10^{9}$\hMsun \citep{Gawiser07}. Considering a
extreme case where all baryonic mass is in the form of stars an under
the assumption that the baryon to dark matter ratio cannot be larger
than the cosmological value $\Omega_{\rm b}/Omega_{\rm dm}\sim 0.2$,
the minimum value for the dark matter halo that we should consider is
precisely $\sim 10^{10}$\hMsun. In turn, this limit help us impose the
minimum value of $f_{\rm occ}=0.1$. Lower values for the occupation
fraction would produce a halo number density well below the observational
constraints. 
...
... \cite{Ouchi2010} prefers a very lowe duty cycle $0.008\pm 0.03$.

...
\ used by \citep{Yamada2012}. These differences  translates
in only two more detected objects in the Hayashino survey (283 LAEs) than in 
Yamada et al (281 LAEs).
...
that are resolved with at least $70$ particles ({\bf REFF???? or support???})
...
We consider that a model is consistent with observations if the two parameters $\gamma$ and $\theta_0$ are

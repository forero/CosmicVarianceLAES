...
A complementary perspective to the range of the allowed values for the
escape fraction is the quantification of the allowed halo mass range
$\Delta M=\log_{10}M_{\rm max} - \log M_{\rm  min}$for LAEs. In this
respect, we find that all the successful models can be  split into
three families:
...
at the low mass end can be readily interepreted in terms of the minimal halo star
formation rate needed to produce the necessary \lya luminosity to be
above a given detection threshold.  However, under the reasonable assumption of star formation rate
...
$\Delta M$ and the third is only narrow in $f_{\rm occ}$. 

Analysis based in observations have been only able to describe models
in the third family, but have so far missed the possibility of the first
two: a narrow mass range and any value for the escape fraction.

Many different observational estimates on the occupation fraction of
LAEs at $z=3$ suggest that the occupation fraction must be low $f_{\rm
  occ}<0.2$. We find that the first and most numerous family of models
is consistent with this suggestion, where also a narrow range for the
halo mass $\Delta M<1.0$ is prefered.

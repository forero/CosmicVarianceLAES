...
%Figure \ref{fig:high_success_rate} shows the number of models that
%have at least $N_{\rm high-P}$ mocks with $P>0.05$. For each model
%${\mathcal M}$ there are 15 different mock surveys.  We discard the
%models that have 14 mocks or less consistent with observations. 
...
best estimates of $\theta_{0}-\beta$ in the parameterization of the
ACF. Blue circles with error bars represent the results from the mocks
and the horizontal line the result from observations reported by
\cite{Ouchi2010} over a field with average number density. The
observational result does not have an uncertainty associated with
$\beta$ because the parameter was fixed in the fit. Only $40$ models
...
$\beta$. Comparing this Figure
\ref{fig:restriction_mock_and_f_occ_corr} with the results shown in Figure
\ref{fig:restriction_mock} we see that the new constraint disfavors
practically all models with $M_{\rm max}>10^{12}$.


Our main conclusions after this comparison is that clustering
information is able to put meaningful constraints on the values of
$M_{\rm max}$. However, the possible values for the occupation
fraction, despite having a strong correlation with $M_{\rm min}$
...
green cross to observations by \citet{Ouchi2010} . The error
...
We end up with $XX$ models, out of $9000$ initial guesses, consistent
with observations. In order to discuss these results we now
define the halo mass range $\Delta M=\log_{10}M_{\rm max} -
...
\item[(2)] High $f_{\rm occ}> 0.3$, $M_{\rm min}\geq 10^{10.8}\hMsun$ and
  low $\Delta M\leq 1.0$: XX models 
\end{itemize}
%
A complete parameter list is presented in Table
\ref{table:firstfamily} and Table \ref{table:secondfamily}. There are
two interesting feature in these families. First, the occupation
fraction can take any value from $0.1$ to $1.0$. Second, the halos
hosting LAEs all have a narrow mass range $\Delta M\leq 1.0$.

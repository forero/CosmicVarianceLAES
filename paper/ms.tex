...
probability $f_{\rm occ}$ if its mass is found within the mass range
...
available are not sufficient to derive a unique and narrow set of
values for the parameter in the model. We find three different
families of models based on their mass range $\Delta M \equiv
\log_{10}M_{\rm max} - \log_{10} M_{\rm min}< 1.0$ dex and occupation
fraction $f_{\rm occ}$.  The dominant familiy is composed by models with a narrow
mass range $\Delta M<1.0$ dex, a low occupation fraction $f_{\rm occ}\leq 0.3$
and a maximum mass $M_{\rm max}< 10^{12}$\hMsun.  This finding
suggests that the most massive dark matter halos at that epoch do not
host the brightest LAEs. This also gives support to observational evidence
concluding that only a small fraction of star forming galaxies can be
actually detected as LAEs. We provide tables and mock catalogs for the
succesful models. 
...
constraints to define the agreement with observations and the
inclusion of the angular correlation function information by
\cite{Hayashino2004}.  
...
All the prefered models have a halo mass range with $M_{\rm
...
A central result in this exploration of paremter espace is the
...
$M_{\rm max}$ for each model become ineffecient in hosting detectable
...
below the imass scale of $10^{11.4}$\hMsun infered as a lower
threshold for the LBGs used in halo abundace matching
studies. Detailed study of high-z LAES will have a possitive impact on
our undertading of the connection of low mass halos to star forming
galaxies.

We forsee that the new observations with new instruments (such as MUSE,
...
Institute for Advance Studies where the first full draft of this paper
was completed.

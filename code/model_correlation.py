...
file=open("model_data.dat")
skiprows=2
for i in range(skiprows):
    file.next()
model_info=np.transpose( [[float(x) for x in ln.split()] for ln in file] )
file.close
...
file=open(ID_file)
ID_data=np.transpose( [[int(x) for x in ln.split()] for ln in file] )
file.close
#ID_data=np.loadtxt(ID_file,dtype='int')

file=open(ID_file)
ks_data=np.transpose( [[float(x) for x in ln.split()] for ln in file] )
file.close
